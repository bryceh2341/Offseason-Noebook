...
combined <- combined %>%
  ...
  mutate(PCTFG3 = ntile(FG3_PCT, 100)/100) %>%
  mutate(PCTCFG3 = ntile(CFGPer, 100)/100) %>%
  arrange(desc(Tot)) %>%
  #slice(1:10) %>%
  filter(PLAYER_NAME %in% c("Stephen Curry", "Seth Curry")) %>%
  select(urlPlayerHeadshot, PLAYER_NAME, FG3A, FG3_PCT, PCTFG3, CFGA, CFGPer, PCTCFG3, Open)
...
combined %>%
  ...
  tab_spanner(
    label = "Open",
    columns = vars(FG3A, FG3_PCT, PCTFG3)
  ) %>%
  tab_spanner(
    label = "Contested",
    columns = vars(CFGA, CFGPer, PCTCFG3)
  ) %>%
  fmt_percent(
